...
function mnistData(dataSet) {
	var points = [];
	var pointLine = [];
	var labelLine = [];
	var pFile;
	if (dataSet === "train") {
		pFile = "http://localhost:5000/data/mnist_train_points.csv";
	} else {
		pFile = "http://localhost:5000/data/mnist_test_points.csv";
	}
	var xhr = new XMLHttpRequest();
    xhr.open("GET", pFile, false);
	xhr.send(null);
	
	if (xhr.status === 200) {
		var pointText = xhr.responseText;
		pointLine = pointText.split("\n");
		for (var j = 0; j < pointLine.length; j++ ){
			if (pointLine[j] !== "") {
				var p = pointLine[j].split(",", 4);
				points.push(new Point([Number(p[0]),Number(p[1]),Number(p[2]), Number(p[3]), -1], "rgb(128,128,128)"));
			}
		}
	}
	return points;
...
function mnistTrainData() {
	return mnistData("train");
}

function mnistTestData() {
